 ...
 */
function agm_bp_profiles_map ($overrides=array(), $limit=100) {
	global $wpdb;
	$limit = (int)$limit ? (int)$limit : 100;
	$handler = new Agm_Bp_Pm_UserPages;
	$user_ids = $wpdb->get_col("SELECT ID from {$wpdb->users} LIMIT {$limit}");
	echo $handler->show_users_on_map($user_ids, $overrides);
}
