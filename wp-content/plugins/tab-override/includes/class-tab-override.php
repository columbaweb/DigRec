 ...
 */
class Tab_Override {
	const VERSION = '3.2.1'; // the current version of the plugin
	...
	public function tab_size_field() {
		?>
		<input type="text" id="tab_override_tab_size" maxlength="3" class="small-text" name="tab_override[tab_size]" value="<?php echo esc_attr( $this->_options['tab_size'] ); ?>" />
	...
	public function create_menu() {
		add_options_page( esc_html__( 'Tab Override Settings', 'tab-override' ), 'Tab Override',
	...
	public function add_scripts( $hook_suffix ) {
		...
		if ( in_array( $hook_suffix, $admin_edit_pages ) ) {
			...
			if ( $page_enabled || ( ! $tabs_default && $tabs_button ) ) {
				...
				wp_enqueue_script( 'taboverride', plugins_url( 'js/taboverride-3.0.2.min.js', $this->_plugin_file ),
					false, '3.0.2', true );
				
				wp_enqueue_script( 'jquery-taboverride', plugins_url( 'js/jquery.taboverride-3.0.0.min.js', $this->_plugin_file ),
					array( 'jquery', 'taboverride' ), '3.0.0', true );
				...
				wp_enqueue_script( 'taboverride-setup', plugins_url( 'js/taboverride-setup-2.0.0.min.js', $this->_plugin_file ),
					$dependencies, '2.0.0', true );
